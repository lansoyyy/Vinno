  ...
  Future<void> _fetchBreakerData(String breakerName, String period) async {
    try {
      ...
      final data = await _statisticsService
          .getHistoricalData(breaker['scbId'], period, metric: 'current');
  ...
  Future<void> _fetchRealTimeData() async {
    try {
      ...
      if (breakerSnapshot.exists) {
        ...
        final double currentValue = (breakerData['current'] ?? 0).toDouble();
            ...
            child: Column(
              children: [
                ...
                SizedBox(
                  ...
                  child: TabBarView(
                    children: [
                      CurrentDay(dailyData: dayData[selectedBreaker] ?? {}),
                      CurrentWeek(weeklyData: weekData[selectedBreaker] ?? {}),
                      CurrentMonth(
                          monthlyData: monthData[selectedBreaker] ?? {}),
                      CurrentYear(yearlyData: yearData[selectedBreaker] ?? {}),
                    ],
                  ),
                ),
                Expanded(
